...
def bank_ingest(_: models.User = Depends(get_current_user)):
    return ingest_bank_transactions()


@router.post("/rd/analyze")
def rd_analyze(payload: RDAnalysisRequest, _: models.User = Depends(get_current_user)):
    return run_rd_analysis(payload.model_dump())


@router.get("/managerio/sync")
def managerio_sync(_: models.User = Depends(get_current_user)):
