...
async def handle_relay(request: Request):
    ...
    # Parse from already-consumed raw bytes — calling request.json() again would fail
    data = json.loads(raw)
    logger.info(f"Relay received payload keys: {list(data.keys()) if isinstance(data, dict) else 'list'}")
